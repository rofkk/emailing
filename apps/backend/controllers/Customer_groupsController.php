...
class Customer_groupsController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        $models = hooks()->applyFilters('backend_controller_customer_groups_option_models', $models);
        ...
        if (request()->getIsPostRequest() && ($attributes = (array)request()->getPost($group->getModelName(), []))) {
            ...
            try {
                ...
                foreach ($models as $key => $model) {
                    ...
                    if ($model instanceof CustomerGroupOptionCampaigns && isset(app_param('POST')[$model->getModelName()]['email_header'])) {
                        $model->email_header = ioFilter()->purify(app_param('POST')[$model->getModelName()]['email_header']);
                    }

                    if ($model instanceof CustomerGroupOptionCampaigns && isset(app_param('POST')[$model->getModelName()]['email_footer'])) {
                        $model->email_footer = ioFilter()->purify(app_param('POST')[$model->getModelName()]['email_footer']);
                    }

                    if ($model instanceof CustomerGroupOptionCommon && isset(app_param('POST')[$model->getModelName()]['notification_message'])) {
                        $model->notification_message = ioFilter()->purify(app_param('POST')[$model->getModelName()]['notification_message']);
                    }

                    if ($model instanceof CustomerGroupOptionSending && isset(app_param('POST')[$model->getModelName()]['quota_notify_email_content'])) {
    ...
    protected function getTabs(array $models): array
    {
        ...
        return hooks()->applyFilters('backend_controller_customer_groups_option_tabs', $tabs, $models);
    }
}
