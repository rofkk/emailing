...
class MoveInactiveSubscribersCommand extends ConsoleCommand
{
    ...
    public function actionIndex($src, $dst, $time, $limit = 1000)
    {
        ...
        $srcList = Lists::model()->findByAttributes([
            'list_uid' => $src,
        ]);
        ...
        $dstList = Lists::model()->findByAttributes([
            'list_uid' => $dst,
        ]);
        ...
        $criteria->compare('DATE(t.date_added)', '<=' . date('Y-m-d', (int)strtotime($time)));
        ...
        while (!empty($subscribers)) {
            foreach ($subscribers as $subscriber) {
                ...
                if (!$subscriber->getIsInactiveInTimePeriod($time)) {
                    $subscribersNotIn[] = (int)$subscriber->subscriber_id;
                ...
                if ($subscriber->moveToList((int)$dstList->list_id, false)) {
                    $success++;
