...
class FileExtensionMimes extends CApplicationComponent
{
    ...
    public function getMimesMap(): CMap
    {
        ...
        if (defined('MW_APP_NAME') && is_file($customFile = Yii::getPathOfAlias(sprintf($this->alias, MW_APP_NAME)) . '.php')) {
            $fileData->mergeWith((array)require($customFile));
        }

        if (defined('MW_APP_NAME') && is_file($customFile = Yii::getPathOfAlias(sprintf($this->alias, MW_APP_NAME) . '-custom') . '.php')) {
            $fileData->mergeWith((array)require($customFile));
        }
        ...
        return $this->_mimesMapping = hooks()->applyFilters('file_extensions_mimes_map', $fileData);
    }
    ...
    protected function getMimesManager(): Mimey\MimeTypes
    {
        ...
        $cacheFile  = Yii::getPathOfAlias('common.runtime') . '/mime.types.compiled.php';
        $saveCache  = true;
