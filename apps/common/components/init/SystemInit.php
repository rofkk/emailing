...
class SystemInit extends CApplicationComponent
{
    ...
    public function _runOnBeginRequest(): void
    {
        ...
        if (!in_array($appName, ['backend', 'console']) && !container()->get(OptionCommon::class)->getIsSiteOnline()) {
        ...
        if (!is_cli()) {
            ...
            // nice urls
            if (container()->get(OptionCommon::class)->getUseCleanUrls()) {
    ...
    public function _reindexGetArray(): void
    {
        ...
        app_param('GET')->mergeWith($_GET);
        $_GET = ioFilter()->stripClean($_GET);
    }
     ...
     * @return void
     */
    protected function checkAppsStoredUrls(): void
    {
        ...
        $currentHash = sha1(basename(__FILE__) . (container()->get(OptionCommon::class)->getUseCleanUrls() ? 'yes' : 'no'));
        $storedHash  = $optionUrl->hash;
        $hashChanged = ($currentHash != $storedHash);

        foreach ($apps as $appName) {
            $storedBaseUrl = $optionUrl->getAppUrlByName($appName);
            ...
            if ($storedBaseUrl != $baseUrl) {
                $optionUrl->saveAttributes([$appName . '_absolute_url' => $baseUrl]);
            }
        }

        if ($hashChanged) {
            $optionUrl->saveAttributes(['hash' => $currentHash]);
        }
    }
