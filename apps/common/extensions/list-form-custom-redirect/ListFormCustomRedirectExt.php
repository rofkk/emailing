...
class ListFormCustomRedirectExt extends ExtensionInit
{
    ...
    public function _saveCustomerData(CAttributeCollection $collection)
    {
        ...
        $model->url         = isset(app_param('POST')[$model->getModelName()]['url']) ? (string)str_replace('&amp;', '&', strip_tags(ioFilter()->purify(app_param('POST')[$model->getModelName()]['url']))) : '';
        $model->list_id     = (int)$list->list_id;
        $model->type_id     = (int)$pageType->type_id;
    ...
    public function _redirectIfNeeded()
    {
        ...
        $list_uid = request()->getQuery('list_uid', '');
        if (empty($list_uid)) {
            return;
        }
        ...
        $subscriber_uid = request()->getQuery('subscriber_uid', '');
